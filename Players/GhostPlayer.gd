...
func _physics_process(delta):
	#old_run = run
	set_col(type == "pb" or type == "" or type == "intro")
	...
	if type == "live":
		...
		data == {"Beginner": {-1: Global.load_from_file("intro_times")}}
...
#			pass
	...
	if run["time"] != 0:
		...
		if not Global.player.finishing:
			physics = Global.player.physics if Global.track_playing != -1 else true
